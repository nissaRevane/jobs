...
class Rental
  ...
  def base_price(price_per_km, price_per_day)
    @price ||= (
      time_component(price_per_day) + distance_component(price_per_km)
    ).to_i
  end
  ...
  def find_options(options)
    options
      .select { |option| option[:rental_id] == id }
  ...
  def time_component(price_per_day)
    remaining_days_to_pay = duration_in_days
